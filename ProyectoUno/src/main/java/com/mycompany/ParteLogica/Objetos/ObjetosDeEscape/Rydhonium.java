/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package com.mycompany.ParteLogica.Objetos.ObjetosDeEscape;

import com.mycompany.ParteLogica.Naves.Nave;

/**
 *
 * @author Kenny
 */
public class Rydhonium extends ObjetoDeEscape{

    public Rydhonium(String nombreDeObjeto, int precioDeObjeto, int cantidadCombustiblePremium, String tipoCombustible) {
        super(nombreDeObjeto, precioDeObjeto, cantidadCombustiblePremium, tipoCombustible);
    }

    @Override
    public void efectoEspecialObjeto(Nave naveElegida) {
    }
    
}
