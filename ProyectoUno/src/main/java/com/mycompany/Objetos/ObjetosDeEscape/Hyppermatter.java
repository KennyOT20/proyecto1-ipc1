/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package com.mycompany.Objetos.ObjetosDeEscape;

import com.mycompany.Naves.Nave;

/**
 *
 * @author Kenny
 */
public class Hyppermatter extends ObjetoDeEscape {

    public Hyppermatter(String nombreDeObjeto, int precioDeObjeto, int cantidadCombustiblePremium, String tipoCombustible) {
        super(nombreDeObjeto, precioDeObjeto, cantidadCombustiblePremium, tipoCombustible);
    }

    @Override
    public void efectoEspecialObjeto(Nave naveElegida) {
    }
    
}
